...
class Knight extends Piece {
  ...
  _moveOptions(gamePieces) {
    ...
    this._validMoves = coordinates.map(this._addingCoordinates).filter(this._outOfBoundsCheck).filter(this._filterMoveOptions(gamePieces)).filter( element => !this._blockingFrom );
    ...
    if (!this._blockingFrom && (playersKing._checkingPieces.length == 1)) {
      ...
      if (elligibleBlockingPieceType.length == 1) {
        ...
        this._validMoves = this._validMoves.filter( element => pieceCheckingKingCoordinates.includes(element) );

        // if it is not a multiple moving piece then only capturing coordinate will be returned
      } else this._validMoves = this._validMoves.filter( element => element == pieceCheckingKing._position );
    ...
    } else if (((!!this._blockingFrom) && (playersKing._checkingPieces.length == 1)) || (playersKing._checkingPieces.length > 1)) this._validMoves = [];
