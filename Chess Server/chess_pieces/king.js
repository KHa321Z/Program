...
class King extends Piece {
  ...
  constructor(jsonObjectOrposition, color) {
    ...
    if ((jsonObjectOrposition instanceof Object) && (color == undefined)) { 
      ...
      super(jsonObjectOrposition, color);
      ...
      this.checkingPieces = [];
      this.blockingCheck = [];
      this.toggleHighlightClass = false;
  ...
  initializingRooksForCastling(gamePieces) {
    ...
    this.leftRook = Object.values(Object.filter( gamePieces, ([location, piece]) => (!!piece && (piece.player == this.player) && (piece.type == "Rook") && (piece.position.split("_")[0] == "1")) ))[0];
    this.rightRook = Object.values(Object.filter( gamePieces, ([location, piece]) => (!!piece && (piece.player == this.player) && (piece.type == "Rook") && (piece.position.split("_")[0] == "8")) ))[0];
  ...
  moveOptions(gamePieces, calledForCheck=false) {
    ...
    this.restrictKingMoves(gamePieces);
  ...
  restrictKingMoves(gamePieces) {
    ...
    for (let pieceType of opponentPiecesTypes) {
      ...
      for (let piece of Object.values(opponentPiecesOfPieceType)) {
        ...
        movesForPieces = (piece.type == "Pawn") ? piece.pawnMovesForOpponentKing() : piece.moveOptions(gamePieces, recursiveMeasureForKing);
  ...
  filterToSeeIfChecksOrBlocks(gamePieces) {
    ...
    for (let piece of Object.values(opponentPieces)) {
      ...
      let pieceValidMoves = piece.moveOptions(gamePieces, calledForCheck);

      for (let line of pieceValidMoves) {
        
        for (let currentMove of line) {

          if (!!gamePieces[currentMove]) {
    ...
    if (this.checkingPieces.length > 0) {
      ...
      this.toggleHighlightClass = true;
      this.isCheck = true;
