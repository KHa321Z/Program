const Game = class {
  ...
  initializeBoard(gamePieces) {
    ...
    for (let piece in this.gamePieces) {
      ...
      if (!!this.gamePieces[piece]) {
        ...
        $(`#${position} span`).html(image);

        $(`#${position} span`).draggable({
          //appendTo: "td", 
          containment: "tbody", 
          //cursor: "crosshair", 
          start: (event, ui) => {
            ...
            let tbody = $("tbody");
            let spanPos = span.position();
            let tdPos = span.parent().position();
            let trPos = span.parent().parent().position();

            let spanPosRelToTbodyTop = trPos.top + tdPos.top + spanPos.top;
            let spanPosRelToTbodyLeft = trPos.left + tdPos.left + spanPos.left;

            // let [mouseX, mouseY] = [event.pageX, event.pageY];
            // let [mouseXInTbody, mouseYInTbody] = [mouseX - tbody.position().top, mouseY - tbody.position().left];

            console.log(span.css("top"))
            console.log(span.css("left"))

            // console.log(event.pageX, event.pageY)
            // console.log(mouseXInTbody, mouseYInTbody)
            console.log(spanPosRelToTbodyTop, spanPosRelToTbodyLeft)

            this.selectedPiece = span.parent().id;
            tbody.append(span);
            ...
            // span.css("top", spanPosRelToTbodyTop);
            // span.css("left", spanPosRelToTbodyLeft);
        ...
        $(`#${position} span`).droppable({
          accept: () => {}, 
          tolerance: "pointer", 
          drop: (event, ui) => {
            console.log(this.selectedPiece)
            console.log(this.gamePieces[event.target.id])
          }
        });
    ...
    /*
    $("td:not(td.notations) span").attr("draggable", true);
    $("td:not(td.notations) span").on("dragstart", event => {this.selectedPiece = this.gamePieces[event.target.id];console.log(event.Originalevent.target.id);});
    $("td:not(td.notations) span").on("dragover", event => event.preventDefault());
    $("td:not(td.notations) span").on("drop", event => {
  ...
  movePiece = event => {
    ...
    } else if (!!this.selectedPiece && ((!targetCellPiece) || (this.playerTurn != targetCellPiece.player)) && selectedPieceOptions.includes(targetCellPosition)) {
