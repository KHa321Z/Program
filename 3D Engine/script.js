...
class Camera {
  ...
  project (point) {
    ...
    let r = this.d / point.z;

    return (new Vector2D(r * point.x, r * point.y))
...
let cube = new Cube(0, 0, 300);

function draw() {
  
  ctx.clearRect(-(canvas.width / 2), -(canvas.height / 2), canvas.width, canvas.height);
  
  cube.rotate(0.005, 0.007, 0);
  cube.drawCube();
...
function render (objects) {
  ...
  for (let object of objects) {
    ...
    for (let face of object.faces) {
      ...
      let vertex0 = camera.project(object.vertices[face[0]]);
      ...
      for (let vertex = 1; vertex < face.length; vertex++) {
        ...
        let nextVertex = camera.project(object.vertices[face[vertex]]);
      ...
      ctx.lineWidth = 1.5;
      ctx.strokeStyle = object.edgeColor;
      ctx.closePath();
      ctx.stroke();
      //ctx.fill();
